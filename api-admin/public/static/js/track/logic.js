/**
 * @file overview logic.js 鹰眼可视化实例js 主文件  文件依赖百度地图JSAPI 请使用高级版本浏览器 强烈推荐Chrome。
 * 基于Baidu Map API 2.0。
 * @Author: xuguanlong
 ...
 * @Author: xuguanlong
 */
define('track/logic', ['track/urls', 'track/message', 'track/track', 'track/draw', 'track/util', 'track/Timeline'], function (urls, message, trackModule, drawModule, Util, timeLineControl) {
    ...
    return {
        ...
        getTraceDetail: function () {
            ...
            tip.hide();
            urls.get('http://gps.bigshot.top/api/gps/gettrace', params, sucCbk);
        },
        // 实时监控加载数据
        loadData: function (pageIndex, before, after) {
            ...
            tip.hide();
            urls.get('http://gps.bigshot.top/api/gps/imeis', params, function (res) {
                if (res.status === 0) {
        ...
        loadTrackHistory: function () {
            ...
            for (var k = 0, l = me._trace.tracks_2.length; k < l; k++) {
                ...
                //TODO
                urls.get('http://gps.bigshot.top/api/gps/getgpsbyimei', params, cbks.success, cbks.before, cbks.fail, cbks.after);
            }
        },
        // 根据历史轨迹点计算活跃度(计算活跃时长)
...
define('track/draw', ['track/canvas', 'track/animation', 'track/util'], function (CanvasModule, AnimationModule, Util) {
    return {
        ...
        drawAttr: function (poi, ctx) {
            ...
            ctx.fillText(txt4, sx + 10, sy + 80);
            ctx.fillText(txt3, sx + 10, sy + 100);
            ctx.restore();
        },
        getaddress:function(lng,lat){
            var geoc = new BMap.Geocoder();
            var point = new BMap.Point(lng, lat);
            var address = geoc.getLocation(point, function (rs) {
                var addComp = rs.addressComponents;
                console.log(addComp);
                return addComp.province + "-" + addComp.city + "-" + addComp.district + "-" + addComp.street;
            });
            return address;
        },
        getLayer: function () {
